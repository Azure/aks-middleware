...
func GetPanicInfo() map[string]string {
...
func GetPanicInfoFunc(p any) (err error) {
	// get the file and line number where the panic occurred
	// panic is terminated by custom recovery function and program continues
	panicInfo := GetPanicInfo()
	// format the error message with the file and line number
	return status.Errorf(codes.Internal, "panic_message: %v, file: %s, line: %s, url: %s", p, panicInfo[FilePathKey], panicInfo[LineNumKey], panicInfo[UrlKey])
}

func GetRecoveryOpts() []recovery.Option {
	opts := []recovery.Option{
		recovery.WithRecoveryHandler(GetPanicInfoFunc),
	}
