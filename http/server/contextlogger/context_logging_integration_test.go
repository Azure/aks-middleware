...
var _ = Describe("OperationRequest and ContextLogger Integration", func() {
	...
	It("should log specified operation request details via context logger", func() {
		...
		Expect(logInfo["ResourceName"]).To(Equal("resourceName"))
		Expect(logInfo["APIVersion"]).To(Equal("2021-12-01"))
		Expect(logInfo["CorrelationID"]).To(Equal("corr-test"))
		Expect(logInfo["AcceptedLanguage"]).To(Equal("en-us"))
		Expect(logInfo["MyCustomHeader"]).To(Equal("extraValue"))
		...
		Expect(logInfo).ToNot(HaveKey("RouteName"))
		Expect(logInfo).ToNot(HaveKey("ResourceType"))
		Expect(logInfo).ToNot(HaveKey("AdditionalHeader"))
	})
